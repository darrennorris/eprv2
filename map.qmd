...
::: {.callout-note title="Box Didático: O Duplo Sentido de 'Mapear'" icon="false"}
...
## Mapa Interativo Campus Marco Zero
...
A figura abaixo apresenta a distribuição espacial das armadilhas fotograficas. (Nota: Uma versão interativa deste mapa está disponível na versão online deste livro [Clique aqui para ver o mapa](https://darrennorris.github.io/eprv2/map.html){target="_blank"}).
...
## Escala e a caracterização da paisagem
...
Resultados com o índice de Shannon (metrica SHDI) nos mostra a heterogeneidade da matriz urbana (@fig-escala-div). Existe uma tendencia positiva clara: a diversidade da paisagem aumenta conforme o raio de amostragem se expande de 12,5m para 100m (@fig-escala-div). É importante notar que, neste caso, o aumento da diversidade não é necessariamente positivo do ponto de vista conservacionista. O aumento do Shannon aqui indica a intrusão de elementos antropogênicos (Uso Humano) na vizinhança da vegetação. A "diversidade" aqui é o registro da mudança entre o ambiente natural e o construído.
...
Para modelos inferenciais (como GLM, GAM ou Modelos de Ocupação), a referência na ecologia é:
Dormann et al. (2013) - "Collinearity: a review of methods to deal with it and a simulation study evaluating their performance".
O que diz: Este estudo testou diversos métodos e concluiu que r>0.7 é o ponto onde as estimativas dos coeficientes tornam-se instáveis e o erro do modelo aumenta significativamente.
...
Muitas valores. para ajudar podeos fazer um resumo para 
...
Com base no gráfico de calor e na tabela cor_summary,  tem um caso claro de redundância métrica. Em ecologia da paisagem, quando duas métricas têm uma correlação acima de 0,90, elas estão medindo essencialmente a mesma coisa.
Aqui está a interpretação detalhada e quais métricas você deve escolher:
1. O Grupo Redundante (Área)
As métricas ca (Class Area), pland (Percentage of Landscape) e cpland (Core Area Percentage of Landscape) estão altíssimamente correlacionadas entre si em todas as escalas (r entre 0,94 e 0,98).
O que está acontecendo: À medida que a área total da classe aumenta, a porcentagem da paisagem e a área de núcleo aumentam quase perfeitamente juntas.
Decisão: Escolha apenas UMA destas.
Recomendação: Fique com a pland. É a métrica padrão na literatura científica por ser normalizada (0-100%) e facilitar a comparação entre áreas de tamanhos diferentes.
2. A Métrica de Configuração (ai)
O ai (Aggregation Index) tem correlações moderadas com a área (r≈0,51 a 0,55).
O que está acontecendo: Isso é bom. Significa que o ai traz uma informação nova que a quantidade de área não explica: se as manchas estão muito próximas/agrupadas ou dispersas.
Decisão: Mantenha o ai. Ele é importante para explicar a conectividade estrutural.
3. A Métrica de Borda (ed)
O ed (Edge Density) apresenta um comportamento interessante de dependência de escala.
Na escala de 12.5m, ele é fortemente correlacionado negativamente com a área (r=−0.86).
Na escala de 100m, essa correlação cai para quase zero (r=−0.12).
O que está acontecendo: Em escalas pequenas, a borda e a área estão ligadas de forma simples. Em escalas maiores, a paisagem se torna mais complexa e a densidade de bordas passa a ser independente da quantidade de área.
...
Para evitar problemas de multicolinearidade em suas análises estatísticas futuras, eu recomendo manter este conjunto de 3 métricas independentes:
pland (Representa a Quantidade: quanto de habitat existe).
ai (Representa a Agregação: como esse habitat está agrupado).
ed (Representa a Borda: quanta interface existe com a matriz).
O que descartar:
Descarte ca e cpland (pois a pland já explica quase 100% da variação delas).
Nota sobre a escala:
Observe que no seu gráfico de 100m, os quadrados estão muito mais "claros" (cores mais fracas), exceto entre pland e cpland. Isso indica que, conforme você aumenta o buffer, as métricas tornam-se mais independentes uma da outra, o que é ótimo para modelos estatísticos.
