...
A avaliação da escala de efeito revela respostas ecológicas fortemente contrastantes entre as espécies quando aplicamos o princípio da parcimônia à seleção de modelos. Para os cães, a inclusão da proporção de uso humano na paisagem (métrica PLAND) gera modelos com desempenho expressivamente superior ao modelo nulo (intercept-only) em todas as escalas avaliadas, garantindo um ganho informacional autêntico ($\Delta AICc \gg 2$). Esse poder explicativo atinge seu pico preditivo nas microescalas (12,5 m a 50 m), confirmando que a ocorrência de cães é fortemente estruturada pela infraestrutura antrópica imediata. Em franca oposição, a modelagem para os gatos demonstra que a variável de paisagem não agrega poder explicativo válido. Nas escalas de 12,5 m a 50 m, a inclusão da métrica resulta em valores de AICc piores do que o modelo nulo, indicando que a complexidade adicionada prejudica a estimativa. Mesmo na escala de 100 m, onde se observa a melhor performance para a espécie, a diferença em relação ao modelo nulo não atinge o limiar mínimo exigido ($\Delta AICc > 2$) para justificar o parâmetro adicional. Conclui-se, portanto, que enquanto a distribuição dos cães reflete ativamente a configuração espacial da área construída, a ocorrência de gatos é estatisticamente indistinguível de um padrão de prevalência constante (acaso), sugerindo que outros mecanismos orientam a sua ocupação no campus.
...
O pacote logistf é incrivelmente robusto para resolver problemas de pequenas amostras, mas sua função predict() pode ser um pouco "teimosa" quando alimentada com dados novos. Para contornar isso de forma elegante, usaremos álgebra de matrizes nativa do R dentro do nosso fluxo purrr::map. É um truque avançado, mas o código resultante fica limpo e instrutivo.
...
I. O Script: Split-Apply-Combine com Interação e Firth Neste bloco, vamos criar um "cenário artificial" (expand_grid) e empurrá-lo para dentro dos nossos modelos por meio do pacote purrr.
